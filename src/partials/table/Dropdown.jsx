...
function Dropdown({ handleSelectedOption, limit }) {
  ...
  useEffect(() => {
    if (limit) {
      let _index = options.findIndex((item) => item.period === limit);
      setSelected(_index);
    }
  }, [limit])
  ...
  const handleOptions = (option) => {
    ...
    handleSelectedOption(option.period);
  ...
  return (
    <div className="relative inline-flex">
      <button
        ref={trigger}
        className="btn justify-between min-w-30 bg-white border-slate-200 hover:border-slate-300 text-slate-500 hover:text-slate-600"
